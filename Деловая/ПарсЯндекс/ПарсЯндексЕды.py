    ...
    for idx, href in enumerate(arr):
        ...
        name = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR,
                                        '[class="RestaurantHeader_name UiKitText_root UiKitText_HeaderLoose UiKitText_Bold UiKitText_Text"]'))
        )
        ...
        for i in range(0, scroll_height, scroll_step):
            ...
            pos_name = BeautifulSoup(categories_body, 'html.parser').find_all(
                class_='UiKitDesktopProductCard_name UiKitText_root UiKitText_Body2Loose UiKitText_Regular UiKitText_Text')
            pos_cost = BeautifulSoup(categories_body, 'html.parser').find_all(
                class_='UiKitDesktopProductCard_priceWrapper')
            ...
            for i in range(len(pos_name)):
                if f'{pos_name[i].text} | {pos_cost[i].text} | {pos_dis[i].get("description")} | {pos_pict[i].get("src")}' in arr_pos:
                    ...
                    arr_pos.append(
                        f'{pos_name[i].text} | {pos_cost[i].text} | {pos_dis[i].get("description")} | {pos_pict[i].get("src")}')
