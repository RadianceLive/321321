...
for p in range(10):
    ...
    for product in elements:
        ...
        focus = soup.find('span', string='Направленность').find_next('div',
                                                                     class_='b-characteristics-tab__characteristics-value').get_text(
            strip=True)
        brand = soup.find('span', string='Бренд').find_next('div',
                                                            class_='b-characteristics-tab__characteristics-value').get_text(
            strip=True)
        packed = soup.find('span', string='Упаковано').find_next('div',
                                                                 class_='b-characteristics-tab__characteristics-value').get_text(
            strip=True)
        weight = soup.find('span', string='Вес').find_next('div',
                                                           class_='b-characteristics-tab__characteristics-value').get_text(
            strip=True)
        type_of_feed = soup.find('span', string='Тип корма').find_next('div',
                                                                       class_='b-characteristics-tab__characteristics-value').get_text(
            strip=True)
