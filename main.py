...
def parse():
    ...
    for url in main_url:
        ...
        for p in range(int(pages)):
            ...
            html=driver.find_element(By.CLASS_NAME, 'center_div').find_element(By.CLASS_NAME, 'open_hours_row').get_attribute('innerHTML')
            ...
            for c in range(len(data_id)):
                ...
                name = driver.find_element(By.ID, f'td_{data_id[c]}').find_element(By.CSS_SELECTOR,'a[class="title"]').text
                work = driver.find_element(By.CLASS_NAME, 'center_div').find_element(By.CLASS_NAME, 'open_hours_row').find_elements(By.TAG_NAME, 'td')[c].text 
                phone = driver.find_element(By.CLASS_NAME, 'center_div').find_element(By.CLASS_NAME, 'phone_row').find_elements(By.TAG_NAME, 'td')[c].text  
                type_res = driver.find_element(By.CLASS_NAME, 'center_div').find_element(By.CLASS_NAME, 'type_row').find_elements(By.TAG_NAME, 'td')[c].text
                print(len(driver.find_element(By.CLASS_NAME, 'center_div').find_element(By.CLASS_NAME, 'type_row').find_elements(By.TAG_NAME, 'td')))
                kitchen = driver.find_element(By.CLASS_NAME, 'center_div').find_element(By.CLASS_NAME, 'cuisine_row').find_elements(By.TAG_NAME, 'td')[c].text
                adress = driver.find_element(By.CLASS_NAME, 'center_div').find_element(By.CLASS_NAME, 'address_row').find_elements(By.TAG_NAME, 'td')[c].text
                country = url.split('-')[1]
